...
A modern, header-only C++23 web framework with high-performance routing and async HTTP/WebSocket support.

## ✨ Key Features

- **Simple & Intuitive** - Return strings directly: `return "pong";`
- **Header-Only** - Just include and use, no compilation needed
- **Fast Routing** - Trie-based algorithm with O(k) complexity
- **Auto JSON Parsing** - Request bodies parsed automatically
- **Cookie Support** - Easy `get` and `set` cookie helpers
- **WebSocket Built-in** - Automatic upgrade detection
...
int main() {
    ...
    server->get("/ping", [](auto& req) { return "pong"; });
...
That's it! No complex setup, just simple C++ code.

## 📦 Installation

### Windows (Using Included Boost)

This project includes `boost.zip` with pre-built libraries. Just extract and build!

**Step 1: Extract Boost**
```powershell
# Extract boost.zip to project root
# You should see: boost/bin, boost/cmake, boost/include, boost/lib

# Or extract to a custom location like c:\libraries\boost
```
...
# Or for permanent setup (choose one location):
# Option A: In project folder
...
# Or with custom Boost location:
.\build.ps1 -BoostDir "c:\custom\path\to\boost"
```
...
**Requirements:**
- Windows 10/11
- LLVM Clang 16+ ([Download](https://github.com/llvm/llvm-project/releases))
- PowerShell (built-in)
...
# If boost.zip not found, download Boost manually:
...
# Install dependencies
sudo apt update
sudo apt install build-essential libboost-all-dev clang
...
// Route parameters
server->get("/users/:id", [](auto& req) {
    return "User: " + req.get("id");
});

// Query parameters
server->get("/search", [](auto& req) {
    auto query = req.query_params();
    return "Searching for: " + query["q"];
});
```
...
server->post("/api/users", [](auto& req) {
...
server->get("/login", [](auto& req) {
    wolf::response_t res;
    res.body() = "Logged in!";
    wolf::set_cookie(res, "session", "abc123", "/", "", 3600);  // 1 hour
    return res;
});

// Read cookie
server->get("/profile", [](auto& req) {
